...
| Tech Stack mascot display (light) | `./assets/tech-stack-knight-v2-tall.gif` | Tech Stack right cell, 320×1100 vertical display with the 144-frame base loop, appended 32-frame sword chapter, per-frame pixel signal/landing ornaments, and amplified non-sword chapter effects |
| Tech Stack mascot display (dark) | `./assets/tech-stack-knight-v2-tall-dark.gif` | Dark-theme 320×1100 display selected with `<picture>`, using the same 177-frame contract |
| Tech Stack mascot base (light) | `./assets/tech-stack-knight-v2.gif` | Retained 320×320 base loop consumed by the vertical display generator |
| Tech Stack mascot base (dark) | `./assets/tech-stack-knight-v2-dark.gif` | Retained dark 320×320 base loop consumed by the vertical display generator |
...
- **Tech Stack Mascot v3 (ADR 0009, 2026-09):** The Tech Stack cell keeps a 144-frame, 320×320 base loop with 36 key poses, plus a paired vertical display variant recorded in ADR 0012. The original 16-pose loop keeps its registered baseline frames, while two new ten-pose groups extend the sequence: `C01–C10` for evade/counter combat and `D01–D10` for observe/understand/celebrate. The square base remains a 39.41-second loop. The vertical display keeps a 177-frame, 38.62-second stream: the general display rate is `1.15×`, while the C-group punch chapter uses an effective `1.25×` rate so the repeated punches do not slow the sword and celebration chapters. The chapter order remains 104 original frames through the punch group, 32 sword-action frames, one short base-pose bridge, and the remaining 40 original frames containing the celebration group. The chapter uses the same character reference, registered head anchor at `x=132`, `y=115`, generated sword motion, and a separate hard-edge `tech-stack-impact-atlas.png` layer for amplified punch, hand-off, celebration, and landing effects; the punch layer stays visible across all four micro-beats instead of selecting one small tile per frame. Boundary-connected neutral matte is removed from generated sources before registration; the tall compositor also keys the dark source matte and decodes all intermediate PNGs as `RGB` before compositing, preventing an embedded dark panel and RGB/RGBA transition blanks. The final encoders emit every logical frame as one full, opaque canvas—never a transparent differential rectangle—so switching poses never depends on prior-frame disposal state. The tall pipeline starts with the known base frames, inserts sword motion before the celebration group, uses a short bridge at the hand-off, and keeps the original D-group recovery at the end of the cycle. ADR 0002's mascot files remain unchanged as hidden rollback assets and are no longer rendered.

...
| Tech Stack mascot display (light) | `./assets/tech-stack-knight-v2-tall.gif` | Tech Stack right cell, 320×1100 vertical display with the 144-frame base loop, appended 32-frame sword chapter, and per-frame pixel signal/landing ornaments |
| Tech Stack mascot display (dark) | `./assets/tech-stack-knight-v2-tall-dark.gif` | Dark-theme 320×1100 display selected with `<picture>`, using the same 176-frame contract |
...
- **Tech Stack Mascot v3 (ADR 0009, 2026-09):** The Tech Stack cell keeps a 144-frame, 320×320 base loop with 36 key poses, plus a paired vertical display variant recorded in ADR 0012. The original 16-pose loop keeps its registered baseline frames, while two new ten-pose groups extend the sequence: `C01–C10` for evade/counter combat and `D01–D10` for observe/understand/celebrate. The square base remains a 39.41-second loop. The vertical display appends a 32-frame sword-dance chapter after those 144 base frames, producing a 176-frame, 39.20-second stream at `1.15×` playback; the chapter uses the same character reference, registered head anchor at `x=132`, `y=115`, generated sword motion, and restrained blue/gold impact effects. Boundary-connected neutral matte is removed from generated sources before registration. The final encoders emit every logical frame as one full, opaque canvas—never a transparent differential rectangle—so switching poses never depends on prior-frame disposal state. The tall pipeline keeps the familiar base chapter first so GitHub can begin decoding the known loop before the larger generated action chapter arrives. ADR 0002's mascot files remain unchanged as hidden rollback assets and are no longer rendered.

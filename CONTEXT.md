...
- **Currently building**: High-level in-progress projects (AgentOS, Distributed delay-delivery, Dify customization).
...
3. **Local previews:** `preview-profile.html` (EN) and `preview-profile.zh.html` (ZH) are saved GitHub renders. Open both via `open-previews.bat` / `open-previews.ps1`.
...
- **AgentOS (SDD × Harness on Claude Code) 独立公开仓库:** `待确认` [首个 MVP 原型何时达到公开开源标准？在独立公开仓库就绪前，仅保留在 "Currently building" 与 "What I'm learning" 文字中，不提前升级为独立卡片或徽章]

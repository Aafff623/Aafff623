...
- **External dependency audit (2026-09):** AgentCFO banner self-hosted as `assets/agentcfo-banner.webp` (copied from upstream `San-Y108/agent-cfo`, WebP q90) so the profile survives the teammate repo moving or going private. GitHub stats cards intentionally remain on the public `github-readme-stats.shion.dev` endpoint — an accepted exception to the in-repo rule: the cards are informational only, a replacement would mean self-hosting Vercel (banned) or a heavier metrics Action, and the fallback on outage is a broken image, not wrong data. Revisit if the endpoint dies like activity-graph did.

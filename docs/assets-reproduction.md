...
- **Output:** `320×1100`, 176 full opaque frames, infinite loop, and a `39.20-second` timing manifest at `1.15×` playback. The original `320×320` frame is placed at `y=390`; the first 144 frames remain the familiar base loop and the final 32 frames are a sword-dance chapter.
- **Ornaments:** `scripts/art/tech-stack-ornament-sprites.png` is the tracked supplementary pixel-art source sheet. `scripts/tech-stack-tall-decorations.py` crops those sprites and renders 176 transparent overlay frames per theme. The top zone loops a signal constellation, scan point, nodes, and staggered sparks; the bottom zone loops a landing pad, scan line, energy nodes, and staggered sparks. Light and dark themes use separate low-contrast palettes. The ornament layer never overlaps the character frame.
- **Generate:** `node scripts/compose-tech-stack-tall-mascot.js`. The script reads the existing square GIFs, decodes the 144 frames, prepares 32 cleaned/registered sword frames from `scripts/art/tech-stack-sword/` with `scripts/prepare-tech-stack-sword-frames.py`, appends them after the base loop, renders a matching 176-frame ornament overlay, renders an RGB intermediate stream, builds one global palette, encodes complete GIF frames, and restores delays from `scripts/tech-stack-sword-sequence.js`.
...
- **Output:** `scripts/tech-stack-mascot-sequence.js` still defines the 36 key poses and 144-frame square base loop. `scripts/tech-stack-sword-sequence.js` composes that base manifest with 32 `sword-01`–`sword-32` action frames, for 176 frames and `39.20 seconds` in the tall display only. It preserves the frame-to-frame rhythm while applying a uniform `1.15×` playback rate and rounding each delay to GIF centiseconds. The C group keeps the punch cadence; the D group uses progressively longer key holds and transition delays for question, realization, celebration, landing, and settle beats. Light background `#ffffff`; dark background `#0d1117`. Each base frame and prepared sword frame is a full, opaque `320×320` canvas at `(0,0)`; transparent difference rectangles are forbidden because they make a new pose depend on prior-frame compositing.
...
- **Timing pass:** `scripts/compose-tech-stack-tall-mascot.js` rewrites the GIF Graphic Control Extension delays from `scripts/tech-stack-sword-sequence.js`. The manifest applies a uniform `1.15×` speed-up, keeps the existing relative cadence, and uses a measured `4.78-second` sword chapter after rounding to GIF centiseconds. The final binary contains 176 delays totaling `39.20 seconds` for both variants. This changes only playback timing; the decoded RGB frames remain unchanged. Recheck the final output's GIF image descriptors after this pass; all 176 must still be full opaque canvases.

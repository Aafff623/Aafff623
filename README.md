![threetwoa banner](./assets/v7-banner.png)

<h1 align="center">threetwoa</h1>

<p align="center"><i>Code less, Architect more. · 少写代码，多搭架构。</i></p>

<table width="100%">
  <tr>
    <td width="60%" valign="top">
      <p>我是一名大二软件工程学生，正在探索如何把 AI 编程工具（GPT、Claude Code、Codex、MCP、Skills）整合成稳定可复用的工作流。同时作为 Web3 hackathon 前端 contributor，专注于 demo 交付和前端体验打磨。</p>
      <p>I’m a sophomore Software Engineering student exploring how to integrate AI coding tools into reusable workflows. I also contribute frontend work for Web3 hackathon demos, focusing on polished delivery and user experience.</p>
      <ul>
        <li><b>Identity · 身份：</b> AI Workflow Builder · Spec-driven Student Builder · Web3 Hackathon Frontend</li>
        <li><b>Background · 背景：</b> 🎓 太原 · 大二 · 软件工程 · <code>laiyif68@gmail.com</code></li>
        <li><b>Workflow · 工作流：</b> GPT as Brain · Claude Code as Executor · Codex as Reviewer · Human-in-the-loop</li>
      </ul>
    </td>
    <td width="40%" align="center" valign="middle">
      <img src="./assets/hero-knight.png" width="78%" alt="threetwoa hero" />
    </td>
  </tr>
</table>

---

## Highlights · 亮点

<table width="100%">
  <tr>
    <td width="50%" valign="top">
      <h3>🤖 AI Workflow · AI 工作流</h3>
      <p>沉淀 Claude Code / Cursor / Codex / MCP / Skills 多端协作流程，构建可复用的 AI coding system。</p>
    </td>
    <td width="50%" valign="top">
      <h3>⛓️ Web3 Hackathon · 黑客松</h3>
      <p>参与 AI Web3 共学营 & 黑客松，方向 Smart Account + Session Key，要求测试网 Tx Hash。</p>
    </td>
  </tr>
  <tr>
    <td width="50%" valign="top">
      <h3>🛠️ Full-stack Projects · 全栈项目</h3>
      <p>从 React + Supabase 的 AI 宠物平台到 Java Spring Boot / FastAPI 后端，覆盖前后端完整链路。</p>
    </td>
    <td width="50%" valign="top">
      <h3>📋 Spec-driven · 规范驱动</h3>
      <p>用 PRD、CONTEXT.md、Skill 规范把单次聊天里的灵感变成可维护、可复现的工程资产。</p>
    </td>
  </tr>
</table>

---

## Tech Stack · 技术栈

<b>Frontend · 前端</b>  
<img src="https://img.shields.io/badge/React-20232a?style=flat-square&logo=react" alt="React" />
<img src="https://img.shields.io/badge/Vue-35495E?style=flat-square&logo=vue.js" alt="Vue" />
<img src="https://img.shields.io/badge/TypeScript-3178c6?style=flat-square&logo=typescript&logoColor=white" alt="TypeScript" />
<img src="https://img.shields.io/badge/Tailwind-06b6d4?style=flat-square&logo=tailwindcss&logoColor=white" alt="Tailwind CSS" />

<b>Backend · 后端</b>  
<img src="https://img.shields.io/badge/Java-007396?style=flat-square&logo=java&logoColor=white" alt="Java" />
<img src="https://img.shields.io/badge/Spring%20Boot-6DB33F?style=flat-square&logo=springboot&logoColor=white" alt="Spring Boot" />
<img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white" alt="Python" />
<img src="https://img.shields.io/badge/FastAPI-009688?style=flat-square&logo=fastapi&logoColor=white" alt="FastAPI" />

<b>AI Workflow · AI 工作流</b>  
<img src="https://img.shields.io/badge/Claude%20Code-d4a27f?style=flat-square&logo=claude" alt="Claude Code" />
<img src="https://img.shields.io/badge/Cursor-000000?style=flat-square&logo=cursor" alt="Cursor" />
<img src="https://img.shields.io/badge/Codex-412991?style=flat-square&logo=openai" alt="Codex" />
<img src="https://img.shields.io/badge/MCP-0969da?style=flat-square" alt="MCP" />

<b>DevOps · 运维</b>  
<img src="https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white" alt="Docker" />
<img src="https://img.shields.io/badge/Nginx-009639?style=flat-square&logo=nginx&logoColor=white" alt="Nginx" />
<img src="https://img.shields.io/badge/MySQL-4479A1?style=flat-square&logo=mysql&logoColor=white" alt="MySQL" />
<img src="https://img.shields.io/badge/Redis-DC382D?style=flat-square&logo=redis&logoColor=white" alt="Redis" />
<img src="https://img.shields.io/badge/Linux-FCC624?style=flat-square&logo=linux&logoColor=black" alt="Linux" />

---

## GitHub Stats · 数据

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Aafff623&show_icons=true&bg_color=ffffff&title_color=0969da&text_color=1f2328&icon_color=f59e0b&hide=prs,issues&count_private=true&hide_border=false&border_color=d1d9e0&card_width=500" alt="GitHub stats" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Aafff623&layout=compact&bg_color=ffffff&title_color=0969da&text_color=1f2328&icon_color=f59e0b&hide_border=false&border_color=d1d9e0&card_width=320" alt="Top languages" />
</p>

---

## Projects · 项目

<table width="100%">
  <tr>
    <td width="33%" valign="top">
      <h3><a href="https://github.com/San-Y108/agent-cfo">AgentCFO</a></h3>
      <p>DAO AI 财务官 / Web3 hackathon 团队项目。负责前端 demo 体验与视觉交付，从组件设计到交互打磨覆盖完整前端链路。</p>
      <p><a href="https://agentcfo-frontend.vercel.app/">Demo</a> · <a href="https://github.com/San-Y108/agent-cfo">Repo</a></p>
      <p><img src="https://img.shields.io/badge/Web3-0969da?style=flat-square" alt="Web3" /> <img src="https://img.shields.io/badge/Frontend-f59e0b?style=flat-square" alt="Frontend" /></p>
    </td>
    <td width="33%" valign="top">
      <h3><a href="https://github.com/Aafff623/pet-adoption-app">PetConnect</a></h3>
      <p>AI 驱动的宠物救助 / 领养 / 健康管理平台。35+ 页面、20+ API 模块、PWA、Supabase、多 LLM 支持，覆盖领养匹配、健康日记、救助任务板等完整功能。</p>
      <p><a href="https://github.com/Aafff623/pet-adoption-app">Repo</a></p>
      <p><img src="https://img.shields.io/badge/React-20232a?style=flat-square&logo=react" alt="React" /> <img src="https://img.shields.io/badge/Supabase-3ecf8e?style=flat-square&logo=supabase" alt="Supabase" /> <img src="https://img.shields.io/badge/PWA-5a0fc8?style=flat-square" alt="PWA" /></p>
    </td>
    <td width="33%" valign="top">
      <h3><a href="https://github.com/Aafff623/web3career-study-track">AI Web3 School · Study Track</a></h3>
      <p>AI Web3 共学营 & 黑客松全链路记录。42/42 预习完成、沉淀 6 个自定义 Skill、WCB API 接入，最终方向 Smart Account + Session Key。</p>
      <p><a href="https://github.com/Aafff623/web3career-study-track">Repo</a></p>
      <p><img src="https://img.shields.io/badge/AI%20×%20Web3-0969da?style=flat-square" alt="AI x Web3" /> <img src="https://img.shields.io/badge/Smart%20Account-f59e0b?style=flat-square" alt="Smart Account" /></p>
    </td>
  </tr>
</table>

---

## Currently Learning · 正在学

- <b>Web3 深入：</b> Smart Account、Session Key、Agentic Wallet、链上交互与测试网部署
- <b>AI 工程化：</b> 把单次 Claude Code 会话沉淀成可复用 Skill、Workflow 和 MCP 工具链
- <b>前端交付：</b> 复杂表单、状态管理、PWA 离线能力与 hackathon demo 的快速迭代

---

## 2026 Goals · 今年目标

<table width="100%">
  <tr>
    <td width="50%" valign="top">
      <h3>🚀 Ship 3+ Projects</h3>
      <p>完成并发布 3 个以上有真实用户或 demo 可演示的项目。</p>
    </td>
    <td width="50%" valign="top">
      <h3>📦 Open Source</h3>
      <p>把 AI 工作流中的通用能力开源成 Skill 或 CLI 工具。</p>
    </td>
  </tr>
  <tr>
    <td width="50%" valign="top">
      <h3>🏆 More Hackathons</h3>
      <p>参与更多 Web3 / AI 黑客松，把想法变成测试网上可运行的 MVP。</p>
    </td>
    <td width="50%" valign="top">
      <h3>📝 Documentation</h3>
      <p>持续输出学习笔记、项目复盘和 AI 编程最佳实践。</p>
    </td>
  </tr>
</table>

---

## Activity · 动态

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph/?username=Aafff623&bg_color=ffffff&color=0969da&line=0969da&point=f59e0b&hide_border=true" alt="activity graph" />
</p>

---

## Contact · 联系

<p align="center">
  <a href="mailto:laiyif68@gmail.com"><img src="https://img.shields.io/badge/Email-laiyif68@gmail.com-0969da?style=flat-square&logo=gmail&logoColor=white" alt="Email" /></a>
  <a href="https://github.com/Aafff623"><img src="https://img.shields.io/badge/GitHub-@Aafff623-0969da?style=flat-square&logo=github&logoColor=white" alt="GitHub" /></a>
</p>

---

<p align="center"><i>Last updated · 最后更新：2026 年 6 月</i></p>

...
- **Section order:** Intro (banner, wordmark, self-intro table + hero, quick links: Email/Blog/Digital Garden) → Contact icons (no heading) → Agent workflow → Tech stack → Competitions → GitHub stats → Classic project → Currently building → What I'm learning → Activity (contribution snake generated in-repo by `.github/workflows/snake.yml`, pushed to the `output` branch; see ADR 0007).
...
- **Tech stack:** 32 shields.io `flat-square` badges across 7 groups (Frontend, Java/Spring, Middleware & microservices, AI/agents, Python, Systems/inference, DevOps). Repo-evidence only: every badge is backed by a repository or competition artifact (see `docs/adr/0006-tech-stack-badges-repo-evidence.md`); new badges require a repo link first. No AI-tools row. Mascot GIFs (light/dark `<picture>`) in the right cell.
